...
import type { LocalLoginDto, LocalLoginRequest, UserDto } from './schemas';
...
export const getSnuttApis = ({
  ...
  ({
    ...
    'POST /v1/auth/login_local': ({ body }: { body: LocalLoginRequest }) =>
      callWithoutToken<
        SuccessResponse<LocalLoginDto> | ErrorResponse<403, 8197>
      >({
        method: 'post',
    ...
    'GET /v1/users/me': ({ token }: { token: string }) =>
      callWithToken<SuccessResponse<UserDto> | ErrorResponse<403, 8194>>({
        method: 'get',
        path: 'v1/user/info',
        token,
      }),
